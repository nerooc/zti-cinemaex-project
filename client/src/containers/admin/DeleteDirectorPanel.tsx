...
const DirectorsPanel = () => {
  ...
  const deleteDirector = async (id) => {
    try {
      const { data } = await axios.post(`/directors/delete/${id}`, {
        headers: {
  ...
  const registerDirectors = (): Promise<DirectorNames> => {
    return new Promise((resolve, reject) => {
      axios
        ...
        .then(({ data }) => {resolve(data); setDirector(data[0].id)})
  ...
  return (
    <>
      ...
      {status === 'success' && value?.length === 0 && (
        <div style={{textAlign: 'center', padding: '50px 0px'}}>No directors available to delete!</div>
        )}
