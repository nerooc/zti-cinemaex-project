...
const MoviesPanel = () => {
  ...
  const deleteMovie = async (id) => {
    try {
      const { data } = await axios.post(`/movies/delete/${id}`, {
        headers: {
  ...
  const registerMovies = (): Promise<MovieNames> => {
    return new Promise((resolve, reject) => {
      axios
        ...
        .then(({ data }) => {resolve(data); setMovie(data[0].id)})
  ...
  return (
    <>
      ...
      {status === 'success' && value?.length === 0 && (
        <div style={{textAlign: 'center', padding: '50px 0px'}}>No movies available to delete!</div>
        )}
